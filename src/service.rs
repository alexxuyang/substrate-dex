...
use dex_v2_runtime::{self, opaque::Block, GenesisConfig, RuntimeApi, WASM_BINARY};
use futures::prelude::*;
...
native_executor_instance!(
	pub Executor,
	dex_v2_runtime::api::dispatch,
	dex_v2_runtime::native_version
);
...
macro_rules! new_full_start {
	($config:expr) => {{
		...
		let builder = substrate_service::ServiceBuilder::new_full::<
			dex_v2_runtime::opaque::Block,
			dex_v2_runtime::RuntimeApi,
		...
		.with_import_queue(|_config, client, mut select_chain, transaction_pool| {
			...
			let (block_import, link_half) =
				grandpa::block_import::<_, _, _, dex_v2_runtime::RuntimeApi, _, _>(
					client.clone(),
					client.clone(),
					select_chain,
				)?;
