...
decl_storage! {
    trait Store for Module<T: Trait> as TokenModule {
        Tokens get(token): map hasher(blake2_128_concat) T::Hash => Option<Token<T::Hash, T::Balance>>;
        Owners get(owner): map hasher(blake2_128_concat) T::Hash => Option<T::AccountId>;
        BalanceOf get(balance_of): map hasher(blake2_128_concat) (T::AccountId, T::Hash) => T::Balance;
        FreeBalanceOf get(free_balance_of): map hasher(blake2_128_concat) (T::AccountId, T::Hash) => T::Balance;
        FreezedBalanceOf get(freezed_balance_of): map hasher(blake2_128_concat) (T::AccountId, T::Hash) => T::Balance;

        OwnedTokens get(owned_token): map hasher(blake2_128_concat) (T::AccountId, u64) => Option<T::Hash>;
        OwnedTokensIndex get(owned_token_index): map hasher(blake2_128_concat) T::AccountId => u64;
