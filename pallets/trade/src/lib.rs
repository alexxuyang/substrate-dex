...
pub trait Trait: token::Trait + system::Trait {
	...
	type Price: Parameter + Member + AtLeast32Bit + Codec + Default + Copy +
...
impl<T: Trait> Module<T> {
	...
	fn calculate_ex_amount(maker_order: &LimitOrder<T>, taker_order: &LimitOrder<T>) -> result::Result<(T::Balance, T::Balance), dispatch::DispatchError> {
		...
		if seller_order.remained_buy_amount <= buyer_order.remained_sell_amount { // seller_order is Filled
			let quote_qty: u128 = 
				Self::into_128(seller_order.remained_buy_amount)? * T::PriceFactor::get() / Self::into_128(maker_order.price)?;
            if Self::into_128(buyer_order.remained_buy_amount)? < quote_qty {
                seller_order_filled = false;
            }
        } else {
			let base_qty: u128 = 
				Self::into_128(buyer_order.remained_buy_amount)? * Self::into_128(maker_order.price)? / T::PriceFactor::get();
        ...
        if seller_order_filled {
			let mut quote_qty: u128 = 
				Self::into_128(seller_order.remained_buy_amount)? * T::PriceFactor::get() / Self::into_128(maker_order.price)?;
			let buy_amount_v2: u128 = quote_qty * Self::into_128(maker_order.price)? / T::PriceFactor::get();
			...
			let mut base_qty: u128 = 
				Self::into_128(buyer_order.remained_buy_amount)? * Self::into_128(maker_order.price)? / T::PriceFactor::get();
			let buy_amount_v2: u128 = base_qty * T::PriceFactor::get() / Self::into_128(maker_order.price)?;
